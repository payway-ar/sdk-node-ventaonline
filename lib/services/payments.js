...
const getFraudDetectionArgs = (fraudDetection) => {
    ...
    const verticalMap = {
        ...
        travel_transaction_data:'travel'
    };
...
module.exports = {
    ...
    paymentInfo: (endpoint, apikey, paymentId, callback) => {
        client = new Client();

        const url = `${endpoint}/payments/${paymentId}`;

        logger.info(` [ paymentInfo ] - requested to endpoint and params: ${url}`);
        ...
        client.get(url, requestOptions, (data, response) => {
            const ret = data;
            const err = "";
            logger.info(` [ paymentInfo ] - response: ${JSON.stringify(data)}`);
            callback(ret, err);
        });
    },
