        ...
        this.healthcheck = function (args, callback) {
            ...
            this.payment = function (args, callback) {
                ...
                const requestData = {
                    ...
                    auth_3ds_data: {}, 
                    amount: decimalToInteger(args.data.amount)
                };

                if (args.data.cardholder_auth_required === true) {
                    const authData = args.data.auth_3ds_data;
                    if (authData && Object.keys(authData).length > 0) {
                        console.log("entreeee al if rarooo")
                        requestData.data.auth_3ds_data = {
                            device_type: authData.device_type,
                            accept_header: authData.accept_header,
                            user_agent: authData.user_agent,
                            ip: authData.ip,
                            java_enabled: authData.java_enabled,
                            language: authData.language,
                            color_depth: authData.color_depth,
                            screen_height: authData.screen_height,
                            screen_width: authData.screen_width,
                            time_zone_offset: authData.time_zone_offset
                        };
                    }
                }
                ...
                client.post(endpoint + "/payments", args, function (data, response) {
                    ...
                    //it is validated that a number comes to just raise the if with the operation
                    if (!isNaN(parseFloat(args.data.amount))) {
                        if (parseFloat(args.data.amount) % 1 !== 0) {
            ...
            this.confirmPayment = function (args, id, callback) {
                client = new Client();
            ...
            this.deleteRefund = function (args, payment_id, refund_id, callback) {
                client = new Client();
                client.delete(endpoint + "/payments/" + payment_id + "/refunds/" + refund_id, args, function (data, response) {
                    ret = data;
                    err = "No hubo error.";
                    if (Buffer.isBuffer(data)) {
                        data = data.toString('utf8');
            ...
            this.threeDSinstruction = function (args, callback) {
                client = new Client();
