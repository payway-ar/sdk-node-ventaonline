...
const cybersourceRetail = require('../lib/services/fraud_detection/cs_retail')
const cyberSourceTicketing = require('../lib/services/fraud_detection/cs_ticketing')
const cybersourceDgs = require('../lib/services/fraud_detection/cs_digitalGoods')
...
module.exports = {
    sdk: function (environment, publicKeyParam, privateKeyParam, grouper, developer, xConsumer = "") {
        ...
        this.cybersourceRetail = (args) => {
            try {
                return cybersourceRetail.generateRetailData(args);
            } catch (error) {
                logger.error('Error en cybersourceRetail: ', error);
                throw error;
            }
        };

        this.cybersourceTicketing = (args) => {
            try {
                return cyberSourceTicketing.generateTicketingData(args);
            } catch (error) {
                logger.error('Error en cybersourceTicketing: ', error);
                throw error;
            }
        };

        this.cybersourceDgs = (args) => {
            try {
                return cybersourceDgs.generatedigitalGoodsData(args);
            } catch (error) {
                logger.error('Error en cybersourceDgs: ', error);
                throw error;
            }
        };
    }
